...
export function SimilarModal({ open, onClose, recipeId, recipeTitle, onStarted }: { open: boolean; onClose: () => void; recipeId: string; recipeTitle: string; onStarted: (runId: string) => void }) {
  ...
  return (
    <AnimatePresence>
      {open && (
        <motion.div className="fixed inset-0 z-50 grid place-items-center p-4 backdrop-blur-[2px]" style={{ background: "var(--backdrop)" }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
          <motion.div className="w-full max-w-lg rounded-[20px] border border-line bg-bg p-8 shadow-[var(--shadow-dialog)]" initial={{ y: 16, scale: 0.98 }} animate={{ y: 0, scale: 1 }} exit={{ y: 16, scale: 0.98 }} onClick={(e) => e.stopPropagation()} role="dialog" aria-label="Generate similar recipes">
            <div className="flex items-start justify-between">
              <div>
                <p className="eyebrow">More like this</p>
                <h2 className="display display-md mt-2">Similar to “{recipeTitle}”</h2>
              </div>
